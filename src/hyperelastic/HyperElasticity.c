...
int main(int argc, char *argv[])
{
  ...
  ierr = IGAAxisInitUniform(axis0,N[0],0.0,1.0,C[0]);CHKERRQ(ierr);
  IGAAxis axis1;
  ierr = IGAGetAxis(iga,1,&axis1);CHKERRQ(ierr);
  ierr = IGAAxisSetDegree(axis1,p[1]);CHKERRQ(ierr);
  ierr = IGAAxisInitUniform(axis1,N[1],0.0,1.0,C[1]);CHKERRQ(ierr);
  IGAAxis axis2;
  ierr = IGAGetAxis(iga,2,&axis2);CHKERRQ(ierr);
  ierr = IGAAxisSetDegree(axis2,p[2]);CHKERRQ(ierr);
  ierr = IGAAxisInitUniform(axis2,N[2],0.0,1.0,C[2]);CHKERRQ(ierr);    
